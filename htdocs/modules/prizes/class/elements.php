...
class PrizesElements extends XoopsObject {
	function PrizesElements(){
		$this->XoopsObject();
...
class PrizesElementsHandler {
	var $db;
	var $db_table;
	var $obj_class = 'PrizesElements';

	function PrizesElementsHandler(&$db) {
		$this->db =& $db;
		$this->db_table = $this->db->prefix('prizes_formelements');
	}
	function &getInstance(&$db) {
		static $instance;
		if (!isset($instance)) {
			$instance = new PrizesElementsHandler($db);
		}
		return $instance;
	}
	function &create() {
	...
	function insertDefaults($form_id){
		...
		include PRIZES_ROOT_PATH.'admin/default_elements.php';
		if( count($defaults) > 0 ){
			$error = '';
