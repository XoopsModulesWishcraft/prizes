<?php
// $Id: elementrenderer.php,v 1.02 2009/06/23 17:30:00 wishcraft Exp $



class PrizesElementRenderer{
	var $_ele;

	function PrizesElementRenderer(&$element){
		$this->_ele =& $element;
	}
			...
			case 'checkbox':
				...
				while( $o = each($options) ){
					...
					if( $other != false && !$admin ){
						$t->addOption($o['key'], _PRIZES_OPT_OTHER.$other);
					}else{
				...
				switch($delimiter){
					...
					default:
						...
						while( $o = each($options) ){
							$other = $this->optOther($o['value'], $form_ele_id);
							if( $other != false && !$admin ){
								$form_ele->addOption($o['key'], _PRIZES_OPT_OTHER.$other);
							}else{
