...
if( !isset($_POST['op']) || $_POST['op'] != 'save' ){
	...
	echo '<div align="center">
			<form action="'.PRIZES_URL.'admin/editelement.php" method="post">
				<b>'._AM_ELE_CREATE.'</b>';
	foreach( $jump as $j ){
		echo "\n".$j->render();
	}
	...
	if( $elements =& $prizes_ele_mgr->getObjects($criteria) ){
		$cell = 'even';
		foreach( $elements as $i ){
			$cell = $cell == 'even' ? 'odd' : 'even';
			$id = $i->getVar('ele_id');
			...
			echo '<tr>';
			echo '<td class="'.$cell.'" colspan="2">'.$i->getVar('ele_caption')."</td>\n";
			echo '<td class="'.$cell.'" rowspan="2" align="center">'.$check_req->render()."</td>\n";
			echo '<td class="'.$cell.'" rowspan="2" align="center">'.$text_order->render()."</td>\n";
			echo '<td class="'.$cell.'" rowspan="2" align="center">'.$check_display->render().$hidden_id->render()."</td>\n";
			echo '<td class="'.$cell.'" nowrap="nowrap" rowspan="2">
			...
			echo '</tr>';
			echo '<tr><td class="'.$cell.'" colspan="2">'.$ele_value->render()."</td>\n</tr>";
		}
	}
	...
	foreach( $ele_id as $id ){
		...
		if( !$prizes_ele_mgr->insert($element) ){
			$error .= $element->getHtmlErrors();
		}
	}
	if( empty($error) ){
		if( $_POST['submit'] == _AM_SAVE_THEN_FORM ){
			redirect_header(PRIZES_ADMIN_URL.'?op=edit&amp;form_id='.$form_id, 0, _AM_DBUPDATED);
		}else{
