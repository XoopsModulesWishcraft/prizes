<?php
// $Id: editelement.php,v 1.02 2009/06/23 17:30:00 wishcraft Exp $


include("admin_header.php");
$prizes_ele_mgr =& xoops_getmodulehandler('elements');
include_once PRIZES_ROOT_PATH.'class/elementrenderer.php';
define('_THIS_PAGE', PRIZES_URL.'admin/editelement.php');
$myts =& MyTextSanitizer::getInstance();
$count =& $prizes_form_mgr->getCount();
if( $count < 1 ){
	redirect_header(PRIZES_ADMIN_URL, 0, _AM_GO_CREATE_FORM);
}
...
switch($op){
	case 'edit':
		...
		if( $clone == true || empty($form_id) ){
			...
			$forms =& $prizes_form_mgr->getObjects(null, 'form_id, form_title');
			foreach( $forms as $f ){
	...
	case 'delete':
		...
		if( empty($_POST['ok']) ){
			...
			$element =& $prizes_ele_mgr->get($ele_id);
			$prizes_ele_mgr->delete($element);
			redirect_header(PRIZES_URL.'admin/elements.php?form_id='.$form_id, 0, _AM_DBUPDATED);
		}
	break;
	case 'save':
		if( !empty($ele_id) ){
