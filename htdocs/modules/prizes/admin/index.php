...
switch($op){
	case 'pages':
		...
		case "delete":
			...
			$page = $prizes_pages_mgr->get($id);	
			if ($prizes_pages_mgr->delete($page))			
		...
		case "save":
			...
			if ($prizes_pages_mgr->insert($page))			
	...
	case 'category':
		...
		case "delete":
			$id = intval($_REQUEST['id']);				
			$category = $prizes_category_mgr->get($id);	
			if ($prizes_category_mgr->delete($category))			
	...
	case 'list':
		...
		if( $forms =& $prizes_form_mgr->getObjects($criteria, 'admin_list') ){
			echo '<form action="'.PRIZES_ADMIN_URL.'" method="post">
			...
			foreach( $forms as $f ){
				...
				echo '
					<tr>
						...
						<td class="odd"><a target="_blank" href="'.PRIZES_URL.'?form_id='.$id.'">'.$f->getVar('form_title').'</a></td>
						<td class="odd" align="center">'.$sendto.'</td>
						<td class="odd"><ul>
							<li><a href="'.PRIZES_ADMIN_URL.'?op=edit&amp;form_id='.$id.'">'
									._AM_FORM_ACTION_EDITFORM.'</a></li>
							<li><a href="elements.php?form_id='.$id.'">'
									._AM_FORM_ACTION_EDITELEMENT.'</a></li>
							<li><a href="'.PRIZES_ADMIN_URL.'?op=edit&amp;clone=1&amp;form_id='.$id.'">'
									._AM_FORM_ACTION_CLONE.'</a></li>
							<li><a href="'.PRIZES_ADMIN_URL.'?op=delete&amp;form_id='.$id.'">'
									._DELETE.'</a></li>
						</ul>'.$ids->render().'</td>
					</tr>';
			}
	...
	case 'edit':
		...
		if( !empty($form_id) ){
			$form =& $prizes_form_mgr->get($form_id);
		}else{
			$form =& $prizes_form_mgr->create();
		}

		$text_form_title = new XoopsFormText(_AM_FORM_TITLE, 'form_title', 50, 255, $form->getVar('form_title', 'e'));

		$group_ids =& $prizes_form_mgr->perm_handler->getGroupIds($prizes_form_mgr->perm_name, $form_id, $xoopsModule->getVar('mid'));
		...
		$output = new XoopsThemeForm($caption, 'editform', PRIZES_ADMIN_URL);
	...
	case 'delete':
		if( empty($_POST['ok']) ){
			...
			xoops_confirm(array('op' => 'delete', 'form_id' => $_GET['form_id'], 'ok' => 1), PRIZES_ADMIN_URL, _AM_FORM_CONFIRM_DELETE);
		}else{
			$form_id = intval($_POST['form_id']);
			if( empty($form_id) ){
				redirect_header(PRIZES_ADMIN_URL, 0, _AM_NOTHING_SELECTED);
			}
			$form =& $prizes_form_mgr->get($form_id);
			$prizes_form_mgr->delete($form);
			$prizes_ele_mgr =& xoops_getmodulehandler('elements');
			$criteria = new Criteria('form_id', $form_id);
			$prizes_ele_mgr->deleteAll($criteria);
			$prizes_form_mgr->deleteFormPermissions($form_id);
			redirect_header(PRIZES_ADMIN_URL, 0, _AM_DBUPDATED);
		}
	break;
	...
	case 'saveorder':
		...
		extract($_POST);
		foreach( $ids as $id ){
			$form =& $prizes_form_mgr->get($id);
			$form->setVar('form_order', $order[$id]);
			$prizes_form_mgr->insert($form);
		}
		redirect_header(PRIZES_ADMIN_URL, 0, _AM_DBUPDATED);
	break;
	...
	case 'saveform':
		...
		if( !$ret = $prizes_form_mgr->insert($form) ){
			$error = $form->getHtmlErrors();
		}else{
			$prizes_form_mgr->deleteFormPermissions($ret);
			if( count($form_group_perm) > 0 ){
				$prizes_form_mgr->insertFormPermissions($ret, $form_group_perm);
			}
			if( !empty($clone_form_id) ){
				$prizes_ele_mgr =& xoops_getmodulehandler('elements');
				$criteria = new Criteria('form_id', $clone_form_id);
				$count = $prizes_ele_mgr->getCount($criteria);
				if( $count > 0 ){
					$elements =& $prizes_ele_mgr->getObjects($criteria);
					foreach( $elements as $e ){
						$cloned =& $e->xoopsClone();
						$cloned->setVar('form_id', $ret);
						if( !$prizes_ele_mgr->insert($cloned) ){
				...
				$prizes_ele_mgr =& xoops_getmodulehandler('elements');
				$error = $prizes_ele_mgr->insertDefaults($ret);
			}
		}
		if( !empty($error) ){
			xoops_cp_header();
			...
			if( $_POST['submit'] == _AM_SAVE_THEN_ELEMENTS ){
				redirect_header(PRIZES_URL.'admin/elements.php?form_id='.$ret, 0, _AM_DBUPDATED);
			}else{
				redirect_header(PRIZES_ADMIN_URL.'?op=edit&amp;form_id='.$ret, 0, _AM_DBUPDATED);
			}
		}
	break;
}
