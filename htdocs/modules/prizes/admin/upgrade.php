<?php
// $Id: upgrade.php,v 1.02 2009/06/23 17:30:00 wishcraft Exp $


include 'admin_header.php';
$version = number_format($xoopsModule->getVar('version')/100, 2);
$count =& $prizes_form_mgr->getCount(new Criteria(1));
if( $version >= 1.2 || $count > 0 ){
	...
	$sql[] = 'ALTER TABLE `'.$xoopsDB->prefix('liaise').'` RENAME `'.$xoopsDB->prefix('prizes_formelements').'`';
	
	$msgs[] = 'Add form_id to elements table...';
	$sql[] = 'ALTER TABLE `'.$xoopsDB->prefix('prizes_formelements')."` ADD `form_id` SMALLINT( 5 ) DEFAULT '1' NOT NULL AFTER `ele_id`";
	
	$msgs[] = 'Change default value of form_id in elements table...';
	$sql[] = 'ALTER TABLE `'.$xoopsDB->prefix('prizes_formelements')."` CHANGE `form_id` `form_id` SMALLINT( 5 ) DEFAULT '0' NOT NULL";
	...
	$sql[] = 
	"CREATE TABLE `".$xoopsDB->prefix('prizes_forms')."` (
	...
	$sql[] = 
	"INSERT INTO `".$xoopsDB->prefix('prizes_forms')."` VALUES (1, '".$method."', ".intval($sendto).", 1, '".$delimiter."', 'Contact Us', '"._SUBMIT."', 'Tell us about your comments for this site.', 'Contact us by filling out this form.', '');";
	...
	if( $error == false ){
		...
		for( $i=1; $i<4; $i++ ){
			...
			$perm->setVar('gperm_name', $prizes_form_mgr->perm_name);
	...
	xoops_cp_header();
	xoops_confirm(array('goupgrade' => 1), PRIZES_URL.'admin/upgrade.php', 'Make sure you have your files and database backuped. Are you really ready to upgrade the module now?', 'Cut the crap and do it');
}
